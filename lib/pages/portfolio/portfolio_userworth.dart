  ...
  Widget userWorth() => 
      ...
      builder: (context,state) {
        if(state is PortfolioLoading){
          return const Center(
           ...
           int reserveStockWorth = calculateUserReservedStocksWorth(state.reservedStocks);
           Int64 cash = state.user.cash;
           Int64 reservedCash = state.user.reservedCash;
           int total = stockWorth + reserveStockWorth + cash.toInt() + reservedCash.toInt();
        ...
        _eachField('Reserved Stock Worth', reserveStockWorth.toString()),
        ...
        Row(
          ...
          const Spacer(),
          Container(padding: const EdgeInsets.only(right: 25,top: 5),
          child: Text(total.toString(),
          style: TextStyle(
            fontSize: 18,
            fontWeight: FontWeight.w800,
            color: total.toInt()>=0 ? secondaryColor: heartRed
              ),
            ),
          )
          ]
    ...
    Widget _holdings(UserWorthLoaded state){
        ...
        mapOfStocks.forEach((stockId, value) {
          if((!stocksReservedMap.containsKey(stockId))&(!stocksOwnedMap.containsKey(stockId))){
    ...
    Widget holdinglist (
      ...
      Map<int,Int64> stocksOwnedMap) => ListView.builder(
      primary: false,
      shrinkWrap: true,
            ...
            Column(
              ...
              children: [
               const Padding(
                ...
                Text(owned.toString(),
                style: TextStyle(
                  color: (owned?.toInt() ?? 0) >= 0 ? secondaryColor :heartRed),
                )
              ],
            ),
            Column(
              ...
              children: [
               const Padding(
                ...
                style: TextStyle(
                  color: (reserved?.toInt() ?? 0) >= 0 ? secondaryColor :heartRed),)
              ],
            ),
            Column(
              mainAxisAlignment: MainAxisAlignment.spaceEvenly,
    ...
    Widget _eachField(String field, String value){
      return Row(children: [
          ...
          const Spacer(),
          Container(padding: EdgeInsets.only(right: 25),
          child: Text(value,
          style: const TextStyle(
            fontSize: 18
              ),
    ...
    int calculateUserReservedStocksWorth( Map<int,Int64> stocksReservedMap){
        ...
        stocksReservedMap.forEach((stockId, value) { 
            worth += value.toInt() * (mapOfStocks[stockId]?.currentPrice.toInt() ?? 0);
        });
        return worth;
    }
